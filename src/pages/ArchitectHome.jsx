export function ArchitectHome({ user, onDiscover, onProfile }) {
  const hour = new Date().getHours();
  const greeting = hour < 12 ? "Good morning" : hour < 17 ? "Good afternoon" : "Good evening";
  const firstName = (user?.name || "there").split(" ")[0];

  const cards = [
    { icon: "🧱", title: "Source Materials", desc: "Browse verified stores to specify materials for your projects.", action: onDiscover, cta: "Source Materials →" },
      ...
      <div style={{maxWidth:720,margin:"0 auto",padding:"20px 16px 80px"}}>
        ...
        <div style={{background:"var(--s1)",border:"1px solid var(--b2)",borderRadius:"var(--rl)",padding:20,marginBottom:12}}>
          ...
          <div style={{fontSize:13,color:"var(--t3)",lineHeight:1.5}}>
            Discover and specify building materials from verified stores across India.
          </div>
        </div>
